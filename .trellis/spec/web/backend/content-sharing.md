...
- Public responses must be one of:
  - `{ available: true, shareId, title, description, content, sourceSnapshot, updatedAt }`
  - `{ available: false }`
...
- Prompt-origin share UIs should render `sourceSnapshot.systemPrompt` and `sourceSnapshot.userPrompt` collapsed by default.
...
- Route test: create enabled share, public read returns content, disable share, public read returns exactly `{ available: false }`.

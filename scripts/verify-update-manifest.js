...
for (const manifestPath of manifestPaths) {
  ...
  for (const entry of entries) {
    ...
    if (typeof entry.size === 'number' && stats.size !== entry.size) {
      console.error(`Size mismatch for ${entry.url}: manifest=${entry.size} actual=${stats.size}`);
      hasError = true;
    }

    const actualSha = sha512Base64(assetPath);
    if (actualSha !== entry.sha512) {
      console.error(`SHA512 mismatch for ${entry.url}: manifest=${entry.sha512} actual=${actualSha}`);
      hasError = true;
    }
  }

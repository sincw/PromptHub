...
> Si quieres probar antes los próximos arreglos, descarga una build desde GitHub `Prereleases` o activa explícitamente el canal de actualizaciones preview en Ajustes.
>
> `0.5.5-beta.1` es una reedición beta histórica creada para devolver un marcador prerelease legible por máquina al antiguo build preview `0.5.5`. Como en semver queda por debajo de `0.5.5` stable, debe tratarse como una build de prueba para descarga manual, no como el destino de actualización predeterminado para usuarios del canal estable.
